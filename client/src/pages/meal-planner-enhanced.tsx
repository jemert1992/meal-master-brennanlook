...
const RecipeItem = ({ recipe, onClick }: { recipe: any, onClick: () => void }) => {
  const [{ isDragging }, drag] = useDrag(() => ({
    ...
    collect: (monitor) => ({
      ...
      if (!monitor.didDrop()) {
        // Handle the case where it wasn't dropped in a valid target
        console.log('Recipe not dropped in valid target');
      }
    }
  }));
  ...
  return (
    <motion.div
      ...
      className="p-3 border rounded-lg cursor-move transform hover:translate-y-[-2px] transition-all hover:shadow-md bg-white dark:bg-gray-800"
    >
      <div className="flex items-center mb-1 text-xs text-gray-500">
        <GripVertical className="h-3 w-3 mr-1" />
        <span>Drag to meal slot</span>
      </div>
      
      {recipe.imageUrl && (
        <div className="w-full h-32 mb-2 overflow-hidden rounded-md bg-gray-100 dark:bg-gray-700">
          <img 
            src={recipe.imageUrl} 
